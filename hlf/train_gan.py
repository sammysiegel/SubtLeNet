...
mass_bins = [0,20,40,60,80,120,160,200,300,500,700,1000] # these are arbitrary for now
n_mass_bins = len(mass_bins)-1
def bin_mass(m):
    for ib in xrange(n_mass_bins):
        if m > mass_bins[ib]:
            return ib 
    return n_mass_bins-1
v_bin_mass = np.vectorize(bin_mass)


fields = ['tau32sd','frec'] + ['ecf%i'%i for i in xrange(11)]
x,y,w,bdt,mass = load_data(0.5,0.25,fields)
imass = {k:np_utils.to_categorical(v_bin_mass(v), n_mass_bins) for k,v in mass.iteritems()}
dim = x['train'].shape[1]
...
d_output = Dense(1, activation='sigmoid', name='hlf_disc')(l)

d_model = Model(inputs=inputs, outputs=d_output)
...
# TODO build a loss function, could look like this:
# https://github.com/fchollet/keras/issues/1061
# concatenate [y_pred,mass_pred] and [y_true,mass_true]
# on the generative-nly part can just use cross entropy
# but restrict training to background samples 
