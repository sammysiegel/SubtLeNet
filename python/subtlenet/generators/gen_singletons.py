    ...
    def __call__(collections, 
        ...
        while True: 
            ...
            for c in collections:
                ...
                if either_decorr_label > 0 and not self.train_mask[c]:
                    w[0] = np.zeros((subbatch_size,))


                if decorr_mass or kl_decorr_mass:
                    mass = self.xform_mass(data['singletons'][:,self.msd_index])
                    if decorr_mass:
                        o.append(mass)
                        w.append(w[0] * nprongs[:,config.adversary_mask])
                    if kl_decorr_mass:
                        # inputs get [one-hot mass] + [sample weight]
                        i.append(np.concatenate([mass, 
                                                 np.reshape(w, (subbatch_size,1))], axis=-1))
                ...
                if decorr_pt or kl_decorr_pt:
                    pt = xform_pt(data['singletons'][:,self.pt_index])
                    if decorr_pt:
                        o.append(pt)
                        w.append(w[0] * nprongs[:,config.adversary_mask])
                ...
                if either_decorr_label > 0:
                    if either_decorr_label > 1:
                        ...
                        labels = data['label']
                    label_weight = data[c.weight] if self.variation_mask[c] else np.zeros((subbatch_size,))
                    if decorr_label > 0:
                        o.append(labels)
