...
def generate(collections, partition='train', batch=32, 
    ...
    generators = {c:c.generator(components=['singletons', 'pf', 'sv', c.weight,'truth'],
                                partition=partition, 
                                batch=small_batch, 
    ...
    while True: 
        ...
        for c in collections:
            ...
            if config.limit:
                i = [data['pf'][:,:config.limit,:], data['sv'][:,:config.limit,:]]
            else:
                i = [data['pf'], data['sv']]
            if learn_mass:
        ...
        merged_inputs = []
        NINPUTS = 2 + int(learn_mass) + int(learn_pt) + int(learn_rho)
