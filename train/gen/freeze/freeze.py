...
if args.adv == 'emd':
    ...
    opts = {
            'loss' : args.adv,
            'scale' : 0.5,   # used to be 0.5
            'w_clf' : 0.001, # used to be 0.001, 0.005
...
if args.adv:
    ...
    callback_params = {
            'partial_model' : clf,
            'monitor' : lambda x : 10 * x.get('val_y_hat_loss') - x.get('val_adv_loss'), # semi-arbitrary
    ...
    train.train(adv, args.adv+args.suffix, adv_gen['train'], adv_gen['validation'], callback_params)
