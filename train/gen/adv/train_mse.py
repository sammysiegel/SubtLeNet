...
adversary.compile(optimizer=Adam(lr=0.00025),
                  ...
                  loss_weights=[0.05] + ([10] * len(kin_hats)))
...
def save_and_exit(signal=None, frame=None, name='mse_decorrelated', model=classifier):
    save_classifier(name, model)
    exit(1)
...
adversary.fit_generator(adversary_train_gen, 
                        ...
                        callbacks = [PartialModelCheckpoint(classifier,
                                                            '%s/%s_%s_best.h5'%(modeldir,'mse_decorrelated',APOSTLE), 
                                                            save_best_only=True, 
                                                            verbose=True),
                                     ModelCheckpoint('%s/stack_%s_%s_best.h5'%(modeldir,'mse_decorrelated',APOSTLE), 
                                                     save_best_only=True, 
                                                     verbose=True)],
                        )
save_classifier(name='mse_decorrelated')
