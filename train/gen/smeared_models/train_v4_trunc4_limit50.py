...
system('cp %s smeared_models/train_%s.py'%(argv[0], APOSTLE))
...
def save_classifier(name='classifier', model=classifier):
    model.save('smeared_models/%s_%s.h5'%(name, APOSTLE))
...
classifier.fit_generator(classifier_train_gen, 
                         ...
                         callbacks = [ModelCheckpoint('smeared_models/%s_%s_best.h5'%('classifier',APOSTLE), save_best_only=True, verbose=True)],
                        )
save_classifier()
